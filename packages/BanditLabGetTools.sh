...
if [ "$ARCH" = "aarch64" ]; then
	...
	sudo -u ubuntu sh -c 'wget --no-check-certificate -qO- https://github.com/0CM/BinaryBanditsForensicLab/raw/main/packages/vt-cli_aarch64-20240720.tar.gz| tar -xz -C "$HOME"/tools/vt && sudo ln -s "$HOME"/tools/vt/vt /usr/bin/vt&& sudo chmod 755 "$HOME"/tools/vt/vt'
	...
	sudo -u ubuntu sh -c 'wget --no-check-certificate -qO- https://github.com/VirusTotal/vt-cli/releases/download/1.0.0/Linux64.zip|  bsdtar -xvf- -C "$HOME"/tools/vt && sudo ln -s "$HOME"/tools/vt/vt /usr/bin/vt&& sudo chmod 755 /tools/vt/vt'
